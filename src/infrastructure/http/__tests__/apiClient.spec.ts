...
describe("apiClient", () => {
    ...
    it("should enforce withCredentials globally for cookie-based authentication", () => {
        expect(apiClient.defaults.withCredentials).toBe(true);
    });

    it("should dispatch auth:expired event on 401 response error", async () => {
        const dispatchSpy = vi.spyOn(window, "dispatchEvent");
        const responseInterceptorError = (apiClient.interceptors.response as any).handlers[0]
            .rejected;
    ...
    it("should dispatch network:midflight-error on network connection failure", async () => {
        ...
        const dispatchedEvent = dispatchSpy.mock.calls.find(
            (call) => call[0]?.type === "network:midflight-error",
        );
